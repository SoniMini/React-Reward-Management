...
#         return {"status": "success", "message": "Registration submitted successfully. Your account will be activated after admin approval"}
...
def create_new_carpainters(firstname, lastname, city, mobile):
    try:
        ...
        existing_carpainter = frappe.db.get_value("Carpenter Registration", 
...
def check_carpainter_registration(mobile_no):
    try:
        # Check if the Carpainter with the provided mobile number already exists
        carpainter_by_mobile = frappe.db.exists("Carpenter", {"mobile_number": mobile_no})

        if carpainter_by_mobile:
            return {"is_registered": True, "message": "Carpainter already exists. Please login into your account."}
        else:
            return {"is_registered": False}
    except Exception as e:
        return {"is_registered": False, "message": str(e)}
